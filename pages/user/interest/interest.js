...
Page({
  ...
  cancel(site){
    let bidList = this.data.bidList, listMsg = this.data.listMsg, tradeCountList = this.data.tradeCountList, type = bidList[site]['SHORT_NAME'];
    bidList.splice(site,1);
    tradeCountList.forEach(item=>{
      if (item.SHORT_NAME==type){
        item.COUNT = item.COUNT-1
      }
    })
    listMsg = bidList.length ? listMsg : '暂无数据';
    this.setData({
      bidList,
      listMsg,
      tradeCountList
  ...
  onShow: function() {
    ...
    let outParam=this.data.outParam;
    if (outParam.cancel){
      this.cancel(outParam.site)
    }
  },
